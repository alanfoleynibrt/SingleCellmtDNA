...
java -jar  /path/to/bin/picard.jar CreateSequenceDictionary \
...
for mode in unshifted shifted; do
  ...
  cat data/sample_info.txt | while read sample; do
    ...
    # sort
    java -jar  /path/to/bin/picard.jar SortSam \
...
for mode in unshifted shifted; do
  ...
  cat data/sample_info.txt | while read sample; do
    # Mark duplicates
    java -jar  /path/to/bin/picard.jar MarkDuplicates \
    INPUT=bwa_mapping/$mode/"$sample".bam \
    OUTPUT=bwa_mapping/$mode/duplicate_marked/"$sample".bam \
    METRICS_FILE=/dev/null

    # Add read groups
    java -jar  /path/to/bin/picard.jar AddOrReplaceReadGroups \
...
for mode in unshifted shifted; do
  ...
  cat data/sample_info.txt | while read sample; do
    java -jar  /path/to/bin/picard.jar BuildBamIndex \
    INPUT=bwa_mapping/$mode/added_read_group/$sample.bam \

    sudo docker run --rm -v /path/to/home:/home broadinstitute/gatk3:3.8-0 \
...
for mode in unshifted shifted; do
  ...
  touch results_output/$mode/indel/variant_table/$sample.summary
    if [ "$mode" = "unshifted" ]; then
      java -jar "/path/to/home/snpEff/snpEff.jar" ann \
