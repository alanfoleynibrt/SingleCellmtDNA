# A Complete Workflow: Single Cell mtDNA Sequencing, from Cell Culture to Bioinformatics

## Description
This text file encompasses the initial processing of sequencing files in the fastq.gz format.
Further R scripts are provided in another text document.
...
Picard will need to be independently downloaded.

## Dependencies

snpEff, VarScan, GenomeAnalysisTK, lofreq_star-2.1.2, picard, vcf tools, trimmomatic
...
# Using CriGri_1.0.99 for shifted/unshifted mt sequence.
```
...
bwa index -a is -p  bwa_index/unshifted/cgridx \
  reference_genome/unshifted/cgr_reference.fasta
  
mkdir bwa_index/shifted
bwa index -a is -p  bwa_index/shifted/cgridx \
  reference_genome/shifted/cgr_reference.fasta
  
```

## mtDNA genome mapping
# may need to repath picard file
```bash
...
### mark duplicates for downstream analyses and GATK compatability
```bash
for mode in unshifted shifted; do
...
cat data/sample_info.txt | while read sample; do
  ...
  samtools depth variant_calling/unshifted/base_recalibration/bam_files/$sample.bam > \
  plotting_data/mapping_statistics/coverage/perbase_coverage/unshifted/$sample.perbase.coverage # for R plotting

  samtools depth variant_calling/shifted/base_recalibration/bam_files/$sample.bam > \
  plotting_data/mapping_statistics/coverage/perbase_coverage/shifted/$sample.perbase.coverage # for R plotting
